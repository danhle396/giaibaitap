/**
 * pdf-vision-solver-gemini.ts
 *
 * Giống pdf-vision-solver.ts nhưng dùng Google Gemini (free tier: 1500 req/ngày).
 *
 * Setup:
 *   1. Lấy API key miễn phí tại https://aistudio.google.com/app/apikey
 *   2. Thêm vào .env.local: GEMINI_API_KEY=AIza...
 *
 * Dùng:
 *   npx tsx scripts/pdf-vision-solver-gemini.ts \
...
async function main() {
  ...
  for (const pageNum of pageNums) {
    ...
    try {
      ...
      const { hasExercises, content } = await analyzePageWithGemini(imgBuffer, pageNum);
      ...
      // Tránh rate limit free tier (15 req/min)
