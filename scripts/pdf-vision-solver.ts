/**
 * pdf-vision-solver.ts
 *
 * Pipeline cho PDF dạng ảnh (scan):
 *   1. Render từng trang PDF thành ảnh PNG (dùng pdf-to-img)
 *   2. Gửi từng trang lên Claude Vision
 *   3. Claude đọc bài tập + giải luôn
 *   4. Gom ~4 trang liền nhau thành 1 file .md → content-templates/inbox/
 *
 * Dùng:
 *   ANTHROPIC_API_KEY=xxx npx tsx scripts/pdf-vision-solver.ts \
 *     --pdf="pdfs/lop-10/1. SGK Toán 10 tập 1 Kết nối tri thức.pdf" \
 *     --lop=10 --mon=toan --bo_sach=ket-noi-tri-thuc --tap=1 \
 *     [--pages=5-30]   ← tuỳ chọn: chỉ xử lý trang nào
...
const pagesArg = arg("pages"); // "15-40" hoặc "15,16,17"
...
if (!pdfPath) {
  console.error(
    ...
    "    --lop=10 --mon=toan --bo_sach=ket-noi-tri-thuc --tap=1 \\\n" +
    "    [--pages=5-60] [--out=content-templates/inbox]"
  );
  process.exit(1);
}
...
function buildMdFile(
  ...
  const metaTitle =
    tieu_de.length > 60 ? tieu_de.slice(0, 57) + "..." : tieu_de;
  const metaDesc =
    `Giải bài tập Toán ${lop} Tập ${tap} trang ${pageRange} ${bookName}. Lời giải chi tiết từng bài, đầy đủ các bước.`
      .slice(0, 155);
    ...
    messages: [
      {
        ...
        content: [
          ...
          {
            type: "text",
            text: `Đây là trang ${pageNum} trong SGK Toán lớp ${lop} (${BOOK_NAMES[boSach] ?? boSach}, Tập ${tap}).
...
async function main() {
  if (!process.env.ANTHROPIC_API_KEY) {
    console.error("❌ ANTHROPIC_API_KEY chưa set\n   export ANTHROPIC_API_KEY=sk-ant-...");
    process.exit(1);
  }
  ...
  // Render tất cả trang thành mảng Buffer (1 lần duy nhất)
  ...
  console.log(
    `🔍 Xử lý ${pageNums.length} trang: ${pageNums[0]}–${pageNums[pageNums.length - 1]}\n`
  );

  if (!existsSync(outDir)) await mkdir(outDir, { recursive: true });

  // Render 1 lần duy nhất — pdfToImg chỉ có thể iterate 1 lần
  ...
  async function flush() {
    ...
    console.log(
      `   💾 ${slug}.md — ${pending.length} trang, ~${(md.length / 1000).toFixed(1)}k ký tự`
    );
    created++;
    pending = [];
  }
  ...
  for (const pageNum of pageNums) {
    // Kiểm tra nếu slug đơn đã tồn tại
    ...
    process.stdout.write(
      `⚙  Trang ${pageNum}/${pageNums[pageNums.length - 1]}... `
    );
    ...
    try {
      ...
      const { hasExercises, content } = await analyzePageWithClaude(
        imgBuffer,
        pageNum
      );
