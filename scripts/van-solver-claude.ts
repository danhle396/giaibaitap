/**
 * van-solver-claude.ts
 *
 * Dùng Claude API (token gói Pro) để sinh nội dung Ngữ Văn lớp 6-12:
 *   - Soạn bài (phân tích tác phẩm, trả lời câu hỏi SGK)
 *   - Văn mẫu (bài nghị luận văn học / NLXH)
 *   - Tóm tắt tác phẩm
 *
 * Dùng:
 *   npx tsx scripts/van-solver-claude.ts \
...
function buildPrompt(item: VanItem, loai: string): string {
  ...
  if (loai === "soan-van") {
    return `Bạn là giáo viên Ngữ Văn THCS/THPT xuất sắc. Hãy soạn bài "${item.ten_tac_pham}"${tacGiaStr} trong chương trình Ngữ Văn lớp ${item.lop} bộ sách ${bookName} (chương trình 2018).
...
Yêu cầu: Viết chuẩn xác, sâu sắc nhưng dễ hiểu với học sinh lớp ${item.lop}. Không được bịa đặt nội dung tác phẩm.`;
  ...
  if (loai === "van-mau") {
    return `Bạn là giáo viên Ngữ Văn xuất sắc. Hãy viết các bài văn mẫu về "${item.ten_tac_pham}"${tacGiaStr} trong chương trình Ngữ Văn lớp ${item.lop} (chương trình 2018).
...
Yêu cầu: Văn phong tự nhiên, có cảm xúc, phù hợp học sinh lớp ${item.lop}. Dẫn chứng phải chính xác từ tác phẩm.`;
  ...
  // tom-tat
  return `Hãy viết tóm tắt tác phẩm "${item.ten_tac_pham}"${tacGiaStr} trong chương trình Ngữ Văn lớp ${item.lop} (chương trình 2018).
...
async function main() {
  ...
  if (!apiKey) {
    console.error("❌ ANTHROPIC_API_KEY chưa set trong .env.local");
    process.exit(1);
  }
  ...
  console.log(`\n📚 Ngữ Văn solver — Claude ${CLAUDE_MODEL}`);
  ...
  outer: for (const item of items) {
    for (const loai of LOAI_LIST) {
      ...
      try {
        ...
        const body = await generateWithClaude(prompt);
